...
lasso_bootstrap_variance <- function(data_train, # Original training data
  ...
  for(m in 1:N_boot){
      ...
      bootstrap_preds[, m] <- lasso_predict_ite(model = cv_bs, x1_test = x1_test, x0_test = x0_test)
  }
...
lasso_get_variance_wrapper <- function(second_stage, 
                                 newX,
                                 model_formula,
...
lasso_ipd_spec <- function(covariate_names,
                           outcome = "y",
                           treatment = "treatment",
                           lambda_seq = NULL) {
...
# 2. Training
lasso_ipd_train <- function(data, spec){

  lasso.models <- vector("list", length = length(data))
  ...
  for(l in 1:nstudies){
     ...
     lasso.models[[l]] <- LASSO_fit_model(data = data[[l]], model_formula = spec$model_formula, 
...
# 3. Prediction
predict.ipd_lasso <- function(object,
                              newX,
                              second_stage = "BS",
                              N_boot = 100){
